...
The backend is **multi-scenario by design** — there is no server-wide "active scenario." Each game is bound at creation to the slug the frontend sends (persisted as the game's `scenario_id`), and every request is scoped to its scenario, so one backend serves `cso`, `tnr`, and others concurrently. If the frontend sends no slug (e.g. a manual API call), game creation defaults to `"cso"`:

```
slug from frontend request  →  "cso"
```

For local development against a non-`cso` scenario, set the slug on the **frontend** — it is the single source of truth:
...
`IMPORT_SCENARIO_SLUG` is separate and unrelated: it is used only by `npm run import:scenario` to choose which Airtable scenario to import. It has no effect on runtime game creation.
